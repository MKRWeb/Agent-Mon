...
const monadLogic = Object.freeze({
  keywordMap: {
    ...
    "founders": "The architects of the Monad network are Keone Hon, James Hunsaker, and Eunice Giarta.",
    ...
    "eunice": "Eunice Giarta is the COO and co-founder of Monad Labs. She brings extensive experience from traditional tech and finance.",
    ...
    "testnet": "The Monad testnet is the proving ground for our Parallel EVM and MonadBFT consensus.",
    ...
    "who are you": "I am the Monad Agent, an optimized AI construct operating on the fastest EVM L1.",
...
function startGreeting() {
  ...
  if (gBubble && gBubble.textContent === '') {
     ...
     typeWriterEffect(gBubble, "gmonad! 💜 I am the Monad Hub Agent. Fast, parallelized, and ready. What do you want to build on 10,000 TPS?", 0, () => { 
        isTyping = false; 
...
function clearChat() {
  ...
  document.getElementById('monad-input-text').readOnly = true;
  document.getElementById('monad-send-btn').disabled = true;
  typeWriterEffect(document.getElementById('cleared-bubble'), "Terminal cleared. EVM state reset. Ready for new input.", 0, () => {
     isTyping = false;
     document.getElementById('monad-input-text').readOnly = false;
     document.getElementById('monad-send-btn').disabled = false;
     saveState();
...
async function handleChat() {
  ...
  typingDiv.innerHTML = '<span class="monad-bot-bubble typing-indicator">> processing parallel tx...</span>';
  ...
  if (!foundStatic) {
    try {
      const systemPrompt = "You are the Monad Agent Hub, a hyper-optimized AI assistant running on the Monad network. You are highly knowledgeable about Monad's Parallel EVM, 10,000 TPS capability, and 1-second block times. You use phrases like 'gmonad' naturally. Keep answers tech-focused, extremely concise (under 3 sentences), and professional but confident.";
      ...
      if (response.ok) {
        ...
        reply = "Error: Block execution reverted. Pollinations API rejected the connection.";
      }
     } catch (error) {
       reply = "Connection timeout. Please verify your node sync (Network Error).";
    }
  }
